...
### 3 — Expose KCP and the provisioner (keep both running)

Two port-forwards are needed — run each in a dedicated terminal:

```bash
# Terminal 1 — KCP front-proxy (required to use tenant kubeconfigs)
make kcp-port-forward        # kubectl port-forward -n kcp svc/kcp-front-proxy 6443:8443

# Terminal 2 — provisioner UI
make provisioner-port-forward
```

The tenant kubeconfigs generated by the provisioner point to `https://localhost:6443/...`,
so `kcp-port-forward` must be running whenever you interact with a tenant workspace.
